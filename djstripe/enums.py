...
class EnumMetaClass(type):
    def __init__(cls, name, bases, classdict):
        def _human_enum_values(enum):
            # These enums are maintained by hand and so trail the Stripe API.
            # humanize() is called from __str__, where raising on an unknown
            # value would make the object unprintable; fall back to the raw
            # value instead.
            return cls.__choices__.get(enum, enum)
...
class DisputeStatus(Enum):
    ...
    # Retired by Stripe, but historical Dispute rows still hold it.
...
class PaymentMethodType(Enum):
    ...
    kakao_pay = _("Kakao Pay")
    klarna = _("Klarna")
    konbini = _("Konbini")
    kr_card = _("Korean card")
    link = _("Link")
    mb_way = _("MB WAY")
    mobilepay = _("MobilePay")
    multibanco = _("Multibanco")
    naver_pay = _("Naver Pay")
    nz_bank_account = _("Bank account (New Zealand)")
    oxxo = _("OXXO")
    p24 = _("Przelewy24")
    pay_by_bank = _("Pay by Bank")
    payco = _("PAYCO")
    paynow = _("PayNow")
    paypal = _("PayPal")
    payto = _("PayTo")
...
class TaxIdType(Enum):
    ...
    by_tin = _("BY TIN")
    ca_bn = _("CA BN")
    ca_gst_hst = _("CA GST HST")
    ca_pst_bc = _("CA PST BC")
    ca_pst_mb = _("CA PST MB")
    ca_pst_sk = _("CA PST SK")
    ca_qst = _("CA QST")
    cd_nif = _("CD NIF")
    ch_uid = _("CH UID")
    ch_vat = _("CH VAT")
